...
def count(myList, element):
    return countRec(myList.head, element)
    
def countRec(head, element):
    if head == None:
        return 0
    if head.data == element:
        return 1 + countRec(head.next, element)
    else:
        return countRec(head.next, element)
        
def remove(myList, element):
    if myList.head == None:
        return False
    if myList.head.data == element:
        myList.head = myList.head.next
        myList.size -= 1
        return True
    else:
        boolean = removeRec(myList.head, element)
        if boolean == True:
            myList.size -= 1
        return removeRec(myList.head, element)
        
def removeRec(head, element):
    if head == None:
        return False
    if head.next == None:
        return False
...
def myListToPyList(myList):
    if myList == None:
        return None
    else:
        return myListToPyListRec(myList.head, [])
        
def myListToPyListRec(head, pyList):
    if head == None:
        return pyList
    else:
        pyList.append(head.data)
        return myListToPyListRec(head.next, pyList)

def pyListToMyList(pyList):
    if pyList == []:
        myList = MyList(None, 0, None)
        return myList
    else:
        myList = MyList(None, 0, None)
        return pyListToMyListRec(pyList, myList, 0)
    
def pyListToMyListRec(pyList, myList, index):
    if index > len(pyList) - 1:
        return myList
    else:
        append(myList, pyList[index])
        index += 1
        return pyListToMyListRec(pyList, myList, index)
